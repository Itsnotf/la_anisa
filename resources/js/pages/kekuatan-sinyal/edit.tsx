...
import { KekuatanSinyal } from '@/types';
...
interface Props {
    kekuatanSinyal: KekuatanSinyal;
}

export default function EditKekuatanSinyal({ kekuatanSinyal }: Props) {
    const form = useForm<z.infer<typeof formSchema>>({
        resolver: zodResolver(formSchema),
        defaultValues: {
            name: kekuatanSinyal.name,
            deskripsi: kekuatanSinyal.deskripsi,
        },
    });

    const onSubmit = (data: z.infer<typeof formSchema>) => {
        router.put(`/kekuatanSinyal/${kekuatanSinyal.id}`, data);
    };
    ...
    return (
        ...
        >
            <Head title="Edit KekuatanSinyal" />
            <div className="m-4 flex max-h-fit max-w-full flex-1 flex-col gap-4 rounded-xl border p-4">
                <h1 className="text-xl font-semibold">Edit KekuatanSinyal</h1>
                <Form {...form}>
